"""
Hybrid Planner Version 5.3 - Enhanced Scoring System
"""
...
class HybridPlanner(AbstractPlanner):
    ...
    def initialize(self, initialization: PlannerInitialization) -> None:
        ...
        logger.info(f"Route initialized with {len(self._route_lane_dict)} lanes")
    ...
    def compute_planner_trajectory(self, current_input: PlannerInput) -> AbstractTrajectory:
        ...
        if self._iteration == 0:
            logger.info(
                f"=== SCENARIO START ===\n"
                f"Ego position: ({ego_state.rear_axle.x:.1f}, {ego_state.rear_axle.y:.1f})\n"
                f"Ego heading: {ego_state.rear_axle.heading:.2f} rad ({np.degrees(ego_state.rear_axle.heading):.1f} deg)\n"
                f"Ego speed: {ego_state.dynamic_car_state.speed:.2f} m/s\n"
                f"Route lanes in dict: {len(self._route_lane_dict) if self._route_lane_dict else 0}\n"
                f"Traffic lights: {len(current_input.traffic_light_data) if current_input.traffic_light_data else 0}\n"
                f"Safety fallback threshold: {self._switcher.config.safety_fallback_threshold}"
            )
        ...
        if self._enable_scenario_detection and self._scenario_detector:
            ...
            # Log significant scenarios or periodically
            if (scenario_result.confidence > 0.7 or 
                self._iteration % 20 == 0):
                logger.info(
                    f"[Iter {self._iteration}] Scenario: {scenario_result.scenario_type.value} "
        ...
        # LOG DRIVABLE AREA INFO on first iteration
        if self._iteration == 0:
            logger.info(f"Drivable area polygons: {len(self._drivable_area_map) if self._drivable_area_map else 0}")
            logger.info(f"Route lanes: {len(self._route_lane_dict)}")
    ...
    def _score_trajectory_enhanced(
        ...
        try:
            ...
            # Log enhanced metrics for analysis when base scores are low or equal
            base_score = score_data['total_score']
            if self._iteration % 20 == 0 or base_score < 0.01:
                logger.debug(
                    ...
                    f"consistency={enhanced_metrics['trajectory_consistency']:.3f}, "
                    f"preparedness={enhanced_metrics['maneuver_preparedness']:.3f}"
                )
            ...
            # Don't print full traceback for enhanced metrics errors to reduce log spam
            if self._iteration < 3:
                import traceback
                logger.debug(f"Enhanced metrics traceback:\n{traceback.format_exc()}")
    ...
    def _select_trajectory_enhanced(
        ...
        # LOG WHEN SAFETY FALLBACK IS TRIGGERED OR AVOIDED
        if pdm_score < 0.01 and diffusion_score < 0.01:
            if metadata.get('safety_fallback', False):
                logger.warning(
                    f"[Iter {self._iteration}] SAFETY FALLBACK to PDM - both scores < 0.01\n"
                    f"  PDM enhanced: efficiency={pdm_enhanced.get('trajectory_efficiency', 0):.2f}, "
                    f"social={pdm_enhanced.get('social_compliance', 0):.2f}, "
                    f"spatial={pdm_enhanced.get('spatial_utilization', 0):.2f}\n"
                    f"  Diffusion enhanced: efficiency={diffusion_enhanced.get('trajectory_efficiency', 0):.2f}, "
                    f"social={diffusion_enhanced.get('social_compliance', 0):.2f}, "
                    f"spatial={diffusion_enhanced.get('spatial_utilization', 0):.2f}"
                )
            else:
                logger.info(
                    f"[Iter {self._iteration}] ENHANCED METRICS OVERRIDE - avoided safety fallback!\n"
                    f"  Selected: {selected_planner.value} despite low base scores\n"
                    f"  Enhanced totals: PDM={pdm_enhanced.get('weighted_total', 0):.3f}, "
                    f"Diff={diffusion_enhanced.get('weighted_total', 0):.3f}"
                )
        
        # Log decisions for significant scenarios OR when interesting switches happen
        if ((scenario_result and scenario_result.confidence > 0.7 and self._iteration % 10 == 0) or
            (selected_planner != self.last_selected_planner)):
            
            logger.info(
                f"[Iter {self._iteration}] Selection: {selected_planner.value} "
                f"({'SWITCHED' if selected_planner != self.last_selected_planner else 'maintained'}) | "
    ...
    def _score_trajectory_safe(self, trajectory: AbstractTrajectory, planner_name: str) -> Dict:
        ...
        try:
            ...
            # DETAILED LOGGING for debugging zero scores
            if total_score < 0.01:
                logger.warning(
                    f"{planner_name} extremely low score breakdown: "
                    f"total={total_score:.4f}, "
                    f"no_collision={no_collision:.2f}, "
                    f"drivable_area={drivable_area:.2f}, "
                    f"driving_direction={driving_direction:.2f}, "
                    f"progress={progress:.3f}, "
                    f"ttc={ttc:.2f}, "
                    f"comfort={comfort:.2f}"
                )
                
                # Log which multiplicative metric is failing
                if no_collision == 0.0:
                    logger.warning(f"{planner_name}: FAILED no_collision check (at-fault collision detected)")
                if drivable_area == 0.0:
                    logger.warning(f"{planner_name}: FAILED drivable_area check (left drivable area)")
                if driving_direction == 0.0:
                    logger.warning(f"{planner_name}: FAILED driving_direction check (>6m against traffic)")
                elif driving_direction == 0.5:
                    logger.warning(f"{planner_name}: PARTIAL driving_direction (2-6m against traffic)")
                
                # Log trajectory details for debugging
                if states.shape[1] > 0:
                    start_state = states[0, 0]
                    end_state = states[0, -1]
                    trajectory_distance = np.hypot(
                        end_state[StateIndex.X] - start_state[StateIndex.X], 
                        end_state[StateIndex.Y] - start_state[StateIndex.Y]
                    )
                    logger.debug(
                        f"{planner_name} trajectory: "
                        f"start=({start_state[StateIndex.X]:.1f}, {start_state[StateIndex.Y]:.1f}), "
                        f"end=({end_state[StateIndex.X]:.1f}, {end_state[StateIndex.Y]:.1f}), "
                        f"distance={trajectory_distance:.1f}m"
                    )
                    
                    # Log velocity information when driving direction fails
                    if driving_direction < 1.0:
                        avg_vx = np.mean(states[0, :, StateIndex.VELOCITY_X])
                        avg_vy = np.mean(states[0, :, StateIndex.VELOCITY_Y])
                        avg_speed = np.mean(np.hypot(
                            states[0, :, StateIndex.VELOCITY_X],
                            states[0, :, StateIndex.VELOCITY_Y]
                        ))
                        logger.debug(
                            f"{planner_name} velocity info: avg_vx={avg_vx:.2f}, "
                            f"avg_vy={avg_vy:.2f}, avg_speed={avg_speed:.2f} m/s"
                        )
    ...
    def _trajectory_to_states(self, trajectory: AbstractTrajectory) -> np.ndarray:
        ...
        # LOG TRAJECTORY CONVERSION DETAILS on first few iterations
        if self._iteration < 3:
            logger.debug(
                f"Converting trajectory: sampled_states={len(sampled_states)}, "
                f"num_states={num_states}, proposal_poses={self._proposal_sampling.num_poses}"
            )
        ...
        # SANITY CHECK on first iteration
        if self._iteration == 0 and num_states > 0:
            logger.debug(
                f"First trajectory state: pos=({states[0, 0, StateIndex.X]:.1f}, "
                f"{states[0, 0, StateIndex.Y]:.1f}), "
                f"vel=({states[0, 0, StateIndex.VELOCITY_X]:.2f}, "
                f"{states[0, 0, StateIndex.VELOCITY_Y]:.2f})"
            )
    ...
    def _initialize_route_info(self, route_roadblock_ids: List[str]) -> None:
        ...
        if not route_roadblock_ids:
            logger.warning("No route roadblock IDs provided")
        ...
        if centerline_discrete_path:
            ...
            # Dummy centerline
            logger.warning("No centerline path found, using dummy centerline")
            states = [StateSE2(0, 0, 0), StateSE2(10, 0, 0), StateSE2(20, 0, 0)]
            self._centerline = PDMPath(states)
    
    def __del__(self):
        """Log summary statistics when planner is destroyed"""
        try:
            if hasattr(self, '_switcher') and hasattr(self._switcher, 'safety_fallback_count'):
                logger.info(
                    f"=== SCENARIO END SUMMARY ===\n"
                    f"Total iterations: {self._iteration if hasattr(self, '_iteration') else 'unknown'}\n"
                    f"Safety fallbacks: {self._switcher.safety_fallback_count}\n"
                    f"Total switches: {self._switcher.total_switches}\n"
                    f"Final planner: {self._switcher.current_planner.value}"
                )
        except:
