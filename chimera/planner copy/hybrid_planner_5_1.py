...
Improves upon v5 with graduated scoring and scenario-aware metrics
"""
...
# Import our enhanced scorer
from chimera.planner.enhanced_pdm_scorer import EnhancedPDMScorer, ScenarioType

# Import our switcher
from chimera.planner.leaky_ddm_switcher import (
    ConservativeOracleSwitcher, ConservativeOracleConfig, PlannerType,
    ScenarioType as SwitcherScenarioType
)
...
class ScenarioDetectionResult:
    ...
    scenario_type: SwitcherScenarioType
    confidence: float
...
class RobustScenarioDetector:
    ...
    def __init__(self, map_api):
        ...
        self.last_scenario = SwitcherScenarioType.UNKNOWN
    ...
    def detect_scenario(
        ...
        try:
            ...
            return ScenarioDetectionResult(
                scenario_type=SwitcherScenarioType.UNKNOWN,
                confidence=0.0,
                features={},
    ...
    def _detect_scenario_validated(self, features: Dict) -> ScenarioDetectionResult:
        ...
        if (features['longitudinal_accel'] < -0.6 and 
            ...
            return self._result(SwitcherScenarioType.STOPPING_WITH_LEAD, 0.9, features, "Stopping with lead")
        
        # STATIONARY_IN_TRAFFIC
        if features['ego_speed'] < 0.1 and features['vehicles_within_8m'] > 6:
            return self._result(SwitcherScenarioType.STATIONARY_IN_TRAFFIC, 0.9, features, "Stationary in traffic")
        ...
        if (features['ego_speed'] > 0.1 and 
            ...
            return self._result(SwitcherScenarioType.WAITING_FOR_PEDESTRIAN, 0.85, features, "Waiting for pedestrian")
        
        # TRAVERSING_PICKUP_DROPOFF
        if features['in_pickup_dropoff'] and features['ego_speed'] > 0.1:
            return self._result(SwitcherScenarioType.TRAVERSING_PICKUP_DROPOFF, 0.85, features, "Pickup/dropoff area")
        
        # BEHIND_LONG_VEHICLE
        if features['behind_long_vehicle']:
            return self._result(SwitcherScenarioType.BEHIND_LONG_VEHICLE, 0.85, features, "Behind long vehicle")
        ...
        if (features['ego_speed'] > 3.5 and 
            ...
            return self._result(SwitcherScenarioType.FOLLOWING_LANE_WITH_LEAD, 0.9, features, "Following lead")
        
        # HIGH_MAGNITUDE_SPEED
        if features['ego_speed'] > 9.0 and features['acceleration_magnitude'] < 1.0:
            return self._result(SwitcherScenarioType.HIGH_MAGNITUDE_SPEED, 0.9, features, "High speed cruise")
        
        # LOW_MAGNITUDE_SPEED
        if (0.3 < features['ego_speed'] < 1.2 and 
            features['acceleration_magnitude'] < 1.0 and
            not features['in_pickup_dropoff']):
            return self._result(SwitcherScenarioType.LOW_MAGNITUDE_SPEED, 0.8, features, "Low speed maneuver")
        
        # HIGH_LATERAL_ACCELERATION
        if (1.5 < features['lateral_accel'] < 3.0 and 
            features['yaw_rate'] > 0.2 and
            not features['at_intersection']):
            return self._result(SwitcherScenarioType.HIGH_LATERAL_ACCELERATION, 0.8, features, "High lateral accel")
        
        # CHANGING_LANE
        if features['is_changing_lane']:
            return self._result(SwitcherScenarioType.CHANGING_LANE, 0.8, features, "Changing lane")
        
        # NEAR_MULTIPLE_VEHICLES
        if features['ego_speed'] > 6.0 and features['moving_vehicles_within_8m'] > 6:
            return self._result(SwitcherScenarioType.NEAR_MULTIPLE_VEHICLES, 0.8, features, "Near multiple vehicles")
        
        # INTERSECTION SCENARIOS
        if features['at_intersection'] and features['ego_speed'] > 0.1:
            if features['is_turning_left']:
                return self._result(SwitcherScenarioType.STARTING_LEFT_TURN, 0.8, features, "Starting left turn")
            elif features['is_turning_right']:
                return self._result(SwitcherScenarioType.STARTING_RIGHT_TURN, 0.8, features, "Starting right turn")
            elif features['has_traffic_light']:
                return self._result(SwitcherScenarioType.STARTING_STRAIGHT_INTERSECTION, 0.8, features, "Straight intersection")
        
        # Default - lower confidence
        return self._result(SwitcherScenarioType.UNKNOWN, 0.3, features, "No specific scenario")
    ...
    def _result(self, scenario_type: SwitcherScenarioType, confidence: float, 
...
class HybridPlanner(AbstractPlanner):
    """
    Version 6 Hybrid Planner - Enhanced Scoring System
    Improves upon v5 with graduated scoring and scenario-aware metrics
    """
    ...
    def __init__(
        ...
        """Initialize hybrid planner with enhanced scoring"""
        ...
        self._enable_enhanced_scoring = enable_enhanced_scoring
        ...
        logger.info(f"Initialized HybridPlanner v6 (Enhanced Scoring, enhanced={enable_enhanced_scoring})")
    
    def initialize(self, initialization: PlannerInitialization) -> None:
        """Initialize all components"""
        logger.info("Initializing HybridPlanner v6...")
        ...
        # Initialize enhanced PDM scorer
        self._scorer = EnhancedPDMScorer(
            proposal_sampling=self._proposal_sampling,
            enable_graduated_scoring=self._enable_enhanced_scoring
        )
    ...
    def _execute_pdm_planner(self, current_input, current_time, scenario_result):
        ...
        try:
            ...
            # Score trajectory with enhanced scorer
            score_data = self._score_trajectory_enhanced(trajectory, 'pdm', scenario_result)
    ...
    def _execute_diffusion_planner(self, current_input, current_time, scenario_result):
        ...
        try:
            ...
            # Score trajectory with enhanced scorer
            score_data = self._score_trajectory_enhanced(trajectory, 'diffusion', scenario_result)
    ...
    def _select_trajectory_conservative(
        ...
        """Select trajectory conservatively with enhanced metrics"""
        ...
        # Log decisions for significant scenarios with enhanced metrics
        if (scenario_result and 
            scenario_result.confidence > 0.7 and
            self._iteration % 10 == 0):
            
            pdm_metrics = self.current_pdm_trajectory.enhanced_metrics or {}
            diff_metrics = self.current_diffusion_trajectory.enhanced_metrics or {}
            ...
            logger.info(
                ...
                f"Lane Center: PDM={pdm_metrics.get('lane_centering', 0):.2f} "
                f"Diff={diff_metrics.get('lane_centering', 0):.2f} | "
        ...
        try:
            ...
            if self._enable_enhanced_scoring:
                ...
                try:
                    ...
                    logger.warning(f"Enhanced scoring failed for {planner_name}, falling back to original: {e}")
            ...
            enhanced_metrics = {}
            if self._enable_enhanced_scoring and hasattr(self._scorer, '_extended_weighted_metrics'):
                try:
                    from chimera.planner.enhanced_pdm_scorer import ExtendedWeightedMetricIndex
                    enhanced_metrics = {
                        'lane_centering': float(self._scorer._extended_weighted_metrics[ExtendedWeightedMetricIndex.LANE_CENTERING, 0]),
                        'smoothness': float(self._scorer._extended_weighted_metrics[ExtendedWeightedMetricIndex.TRAJECTORY_SMOOTHNESS, 0]),
                        'near_miss': float(self._scorer._extended_weighted_metrics[ExtendedWeightedMetricIndex.NEAR_MISS, 0]),
                    }
                except:
    ...
    def name(self) -> str:
        return "HybridPlanner_v6_EnhancedScoring"
